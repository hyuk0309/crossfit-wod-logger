...
export default function BrowsePage({ wods, onUpdateWod, onRemoveWod, initialDate }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <section className="card">
        <div className="row">
          <div className="col">
            <label className="muted" htmlFor="browse-date">조회 날짜</label>
            <input id="browse-date" type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} />
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        {wodsForDate.length === 0 ? (
          <p className="muted">아직 등록된 WOD가 없습니다. 상단에서 날짜를 바꿔보거나, 등록 페이지에서 추가하세요.</p>
        ) : (
          <div className="list">
            {wodsForDate.map((w) => (
              <article key={w.id} className="card" style={{ display: 'grid', gap: 8 }}>
                <header className="row" style={{ alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button className="btn danger" onClick={() => onRemoveWod?.(w.id)}>삭제</button>
                  </div>
                </header>
